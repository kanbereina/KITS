...
from kits.translator import LLMTranslator, TranslationError
...
class TestApiKeyValidation:
    ...
    def test_reads_key_from_env(self, monkeypatch):
        ...
        assert LLMTranslator().api_key == "env-key"

    def test_explicit_key_overrides_env(self, monkeypatch):
        monkeypatch.setenv("DEEPSEEK_API_KEY", "env-key")
        assert LLMTranslator(api_key="arg-key").api_key == "arg-key"
...
class TestBuildUserContent:
    def test_numbers_each_line(self):
        ...
        content = LLMTranslator._build_user_content(batch)
...
class TestParseResponse:
    def test_parses_in_order(self):
        content = "0|||你好\n1|||谢谢"
        assert LLMTranslator._parse_response(content, 2) == ["你好", "谢谢"]
    ...
    def test_missing_line_becomes_empty(self):
        content = "0|||你好"
        assert LLMTranslator._parse_response(content, 2) == ["你好", ""]
    ...
    def test_ignores_non_integer_index(self):
        content = "x|||坏序号\n0|||你好"
        assert LLMTranslator._parse_response(content, 1) == ["你好"]
