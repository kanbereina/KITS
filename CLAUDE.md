...
- yt-dlp 音频提取/转码依赖系统 `ffmpeg`，音频切分也依赖 `ffmpeg`，须在 PATH 中。
- yt-dlp 更新频繁，不进入 `pyproject.toml` / `uv.lock` 固定版本，按外部工具处理：用户自行 `pip install -U yt-dlp` 并确保 `yt-dlp` 在 PATH 中。`download` 统一走 `YtDlpDownloader`，默认最佳音频 + 提取 MP3；分片并发默认 5。高级参数通过 `download URL -- <yt-dlp 原生参数>` 或 `--yt-dlp-args` 透传。
...
- `downloader.py` — `YtDlpDownloader`，通过 PATH 中的 `yt-dlp` 命令下载 Twitch / YouTube 等更多平台视频或直播回放，默认提取最佳音频为 MP3。模块**不依赖 torch**；yt-dlp 作为外部工具由用户自行安装/更新，避免随 uv lock 固定过期版本，也避免在 KITS 内维护 yt-dlp 参数映射。
...
- 转字幕：`downloader`(MP3；yt-dlp 后端) →（可选 `separator` 分离人声）→ `transcriber.plan_audio()`（VAD 探人声区间、规划分段，进度条前跑完，返回 `(duration, segments, speech)`）→ `transcriber.transcribe_segments()`（逐段产出 chunk 级 list[Word]）→（可选 `punctuator.restore()` 补标点）→ 每段 `subtitle.segment_sentences()`(list[Sentence]，传入 `speech` 把虚高 end 夹回人声边界) → `subtitle.SrtWriter.append()` 增量写盘。`_audio_to_srt` 是这条流水线，`_maybe_separate` 按 `--separate` 决定是否预处理人声，标点恢复默认开启（`--no-punctuate` 关闭）。
