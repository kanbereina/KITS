...
- `subtitle.py` — 纯函数库，**不依赖 torch/网络**，可独立单测。负责单词级时间戳 → 完整句子的断句、SRT 渲染，以及 SRT 解析（`parse_srt` / `srt_time_to_seconds`，是渲染函数的逆操作，供 translate 读回字幕用）。`Word` / `Sentence` 是贯穿全项目的 TypedDict 数据契约。
...
- 转字幕：`downloader`(MP3) → `transcriber.transcribe()`(list[Word]) → `subtitle.segment_sentences()`(list[Sentence]) → `subtitle.write_srt()`。
...
- 断句参数（`--max-gap` 等）由 `_add_subtitle_args` 在 download / subtitle 间共用。
