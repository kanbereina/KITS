...
__all__ = ["DEFAULT_CONCURRENT_FRAGMENTS", "DEFAULT_YTDLP_COMMAND", "YtDlpDownloader"]

DEFAULT_CONCURRENT_FRAGMENTS = 5
DEFAULT_YTDLP_COMMAND = "yt-dlp"
YTDLP_INSTALL_HINT = "未找到 yt-dlp 命令。请先安装或更新：pip install -U yt-dlp，并确认 yt-dlp 在 PATH 中。"


class YtDlpDownloader:
    """调用 yt-dlp CLI 的通用音频下载器。

    用子进程执行 PATH 中的 `yt-dlp` 命令，避免在 KITS 内维护 yt-dlp 的参数映射。
    ...
    def __init__(
        ...
        ytdlp_command: str = DEFAULT_YTDLP_COMMAND,
    ):
    ...
    def download_from_url(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(YTDLP_INSTALL_HINT) from e
